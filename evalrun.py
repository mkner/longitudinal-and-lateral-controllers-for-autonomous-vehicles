...
# v0.02 formatting output
# v0.01 cp of c1m7 w/ shorter name
...
def grade(waypoints, solution, visualize = False):
    ...
    percent_dists_correct = (np.sum(dists_correct)/N)*100
    
    # ck if velocities within range only for the waypoints that
    # were in range in the first place
    ...
    #percent_vels_correct = 
    # then use that percent for pass/fail
    ...
    print("")
    print('Waypoints within range:, {0:.2f}%'.format(percent_dists_correct))
    print()
