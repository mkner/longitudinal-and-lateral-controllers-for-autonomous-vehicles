...
"""
CARLA loop that runs cotroller client

"""

#mk now is mod8.py
# make live plot dimensions SMALLER 8 inches is too big!
# rem out brake live plotting in great the subplot & update 
# rem out save brake graph to file
...
#import mycontroller
#mk
#from controller2d import *

#mk for backward compatability

import controllers as controller2d 
...
SAVE_TRAJECTORY=True #False # no silver platter!
print()

print("Starting up mod server version "+str(MOD_VERSION)+"...")
print()
...
#SIMWEATHER = WEATHERID["HARDRAINNOON"]  

PLAYER_START_INDEX = 1      # spawn index for player (keep to 1)
FIGSIZE_X_INCHES   = 9#5#8      # x figure size of feedback in inches
FIGSIZE_Y_INCHES   = 5#8      # y figure size of feedback in inches
...
def write_trajectory_file(x_list, y_list, v_list, t_list):
    ...
    file_name = os.path.join(CONTROLLER_OUTPUT_FOLDER, 'trajectory_data.txt')
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        #mk
        ###############################################
        ### the controller object is instanced here 
        ###############################################
        ...
        # directory.
        if reached_the_end:
            print("Vehicle at end of path. Writing out runtime state-space results...")
        else:
       ...
       # store_trajectory_plot(brake_fig.fig, 'brake_output.png')
        ...
        if ( SAVE_TRAJECTORY == True):
            ...
            #print("-----------------------------------")
            print()
            print("Dumping runtime analysis data to file... ")
            #print("-----------------------------------")
            print()
